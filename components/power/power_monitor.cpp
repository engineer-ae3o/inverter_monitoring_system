...
namespace adc {
    ...
    static constexpr uint16_t ADC_FRAME_SIZE                    = 1024;                    // DMA buffer size (power of 2)
    ...
    driver::driver(): zero_current_offset_voltage(0),
        ...
        memset(&measurement_data, 0, sizeof(data_t));
        memset(&current_channel, 0, sizeof(adc_channel_config_t));
        memset(&voltage_channel, 0, sizeof(adc_channel_config_t));
    }
    ...
    bool driver::init(gpio_num_t current_pin, gpio_num_t voltage_pin) {
        ...
        if (!setup_calibration()) {
            ADC_LOGE("Failed to setup calibration");
            return false;
        }
        ...
        processing_task_handle = xTaskCreateStaticPinnedToCore(processing_task, "adc_process", PROC_TASK_STACK_SIZE, this, 
                                                               PROC_TASK_PRIORITY, processing_task_stack, 
                                                               &processing_task_buffer, PROC_TASK_CORE);
    ...
    bool driver::setup_calibration() {
        ...
        adc_cali_line_fitting_config_t cali_config = {
            ...
            .default_vref = 1086UL // default eFuse value gotten from running 'espefuse.py --port <your esp32's port> adc_info'
        };
    ...
    adc_channel_config_t driver::gpio_to_adc_channel(gpio_num_t pin) {
        ...
        // ADC1 channel mapping for corresponding GPIO pin
        switch (pin) {
        case GPIO_NUM_36:
        ...
        case GPIO_NUM_35:
            ...
            break;
        default:
            ADC_LOGE("Invalid ADC GPIO pin number %d", pin);
            break;
        }
    ...
    bool IRAM_ATTR driver::adc_conv_done_callback(adc_continuous_handle_t handle,
    ...
    void driver::processing_task(void* arg) {
        ...
        // Calculate the ACS712 zero current offset voltage at startup
        {
            ...
            for (uint8_t times = 0; times < TIMES_TO_MEASURE_ACS_OFFSET; times++) {
                ...
                if (ret == ESP_OK && out_length > 0) {
                    ...
                    current[times] = driver->calculate_average(driver->current_samples.data(), current_idx);

                    memset(driver->current_samples.data(), 0, sizeof(driver->current_samples));
                }
            }
            float current_avg = driver->calculate_average(current.data(), current.size());
    ...
    void driver::process_adc_data(uint8_t* buffer, uint32_t length) {
        ...
        uint16_t voltage_idx = 0, current_idx = 0;
    ...
    float driver::calculate_average(const float* samples, uint32_t count) {
    ...
    void driver::update_measurements() {
        ...
        // Calculate voltage
        float voltage = calculate_average(voltage_samples.data(), sample_count);
        voltage *= VOLTAGE_DIVIDER_RATIO;

        // Calculate current
        float current = calculate_average(current_samples.data(), sample_count);
        ...
        if (xSemaphoreTake(data_mutex, pdMS_TO_TICKS(TIMEOUT_MS)) == pdTRUE) {
            ...
            measurement_data.timestamp_ms = xTaskGetTickCount() * portTICK_PERIOD_MS;
        ...
        memset(current_samples.data(), 0, sizeof(current_samples));
        memset(voltage_samples.data(), 0, sizeof(voltage_samples));
    }
    ...
    bool driver::get_measurement_data(data_t& data) {
        ...
        if (xSemaphoreTake(data_mutex, pdMS_TO_TICKS(TIMEOUT_MS)) == pdTRUE) {
            if(measurement_data.valid) {
                memcpy(&data, &measurement_data, sizeof(data_t));
                success = true;
