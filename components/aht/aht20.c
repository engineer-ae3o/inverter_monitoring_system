...
static aht20_err_t poll_till_measurement_done(void) {
    ...
    for (int attempts = 0; attempts < MAX_POLL_ATTEMPTS; attempts++) {
        if (i2c_master_receive(dev_handle, &rx_status_byte, sizeof(rx_status_byte), pdMS_TO_TICKS(AHT20_I2C_TIMEOUT_MS)) != ESP_OK) {
...
const char* aht_err_to_string(aht20_err_t err) {
    switch (err) {
...
aht20_err_t aht20_init(gpio_num_t sda, gpio_num_t scl) {
    ...
    const uint8_t tx_init_byte = AHT20_TX_INIT_STATUS_BYTE;
    uint8_t rx_init_status_byte = 0;

    // Sending init byte to get init message from the AHT20
    esp_err_t ret = i2c_master_transmit_receive(dev_handle, &tx_init_byte, sizeof(tx_init_byte), &rx_init_status_byte, sizeof(rx_init_status_byte), pdMS_TO_TICKS(AHT20_I2C_TIMEOUT_MS));
    if (ret != ESP_OK) {
    ...
    if (i2c_master_transmit(dev_handle, tx_buffer, sizeof(tx_buffer), pdMS_TO_TICKS(AHT20_I2C_TIMEOUT_MS)) != ESP_OK) {
    ...
    if (i2c_master_transmit_receive(dev_handle, &tx_init_byte, sizeof(tx_init_byte), &rx_init_status_byte, sizeof(rx_init_status_byte), pdMS_TO_TICKS(AHT20_I2C_TIMEOUT_MS)) != ESP_OK) {
...
aht20_err_t aht20_deinit(void) {
    ...
    if (!is_initialized) {
        AHT_LOGW("AHT20 already in a deinitialized state");
        return AHT_OK;
    }
    ...
    if (!bus_handle) {
        AHT_LOGE("Invalid I2C bus handle");
        return AHT_FAIL;
    }

    esp_err_t ret = i2c_master_bus_rm_device(dev_handle);
    if (ret != ESP_OK) {
        AHT_LOGE("Failed to remove I2C bus device handle");
        return ret;
    }
    dev_handle = NULL;
    ...
    if (ret != ESP_OK) {
        AHT_LOGE("Failed to delete I2C bus");
        return ret;
    }
    bus_handle = NULL;
...
aht20_err_t aht20_read(aht20_data_t* data) {
    ...
    if (!data) {
        AHT_LOGE("Invalid pointer passed");
        return AHT_INVALID_ARGS;
    }
    ...
    // To enforce minimum reading intervals for the AHT20
    ...
    const uint8_t tx_trigger_read_buffer[3] = { AHT20_TRIGGER_CMD_1, AHT20_TRIGGER_CMD_2, AHT20_TRIGGER_CMD_3 };
    
    int attempt = 1;
    for (; attempt <= AHT20_READ_ATTEMPTS; attempt++) {

        AHT_LOGI("Attempt #%d. Starting read.....", attempt);

        if (i2c_master_transmit(dev_handle, tx_trigger_read_buffer, sizeof(tx_trigger_read_buffer), pdMS_TO_TICKS(AHT20_I2C_TIMEOUT_MS)) != ESP_OK) {
            AHT_LOGW("Failed to transmit measurement triggering sequence");
            vTaskDelay(pdMS_TO_TICKS(AHT20_BACKOFF_DELAY_MS));
            continue;
        }
        vTaskDelay(pdMS_TO_TICKS(AHT20_MEASUREMENT_DELAY_MS)); // Delay to allow the AHT20 take measurements

        // Poll till the AHT20's measurement is complete
        if (poll_till_measurement_done() != AHT_OK) {
            AHT_LOGW("Failure during polling");
            vTaskDelay(pdMS_TO_TICKS(AHT20_BACKOFF_DELAY_MS));
            continue;
        }

        uint8_t rx_data_buffer[7];  // Buffer to store measurements from the AHT20

        if (i2c_master_receive(dev_handle, rx_data_buffer, sizeof(rx_data_buffer), pdMS_TO_TICKS(AHT20_I2C_TIMEOUT_MS)) != ESP_OK) {
        ...
        uint32_t raw_hmdt = ((uint32_t)rx_data_buffer[1] << 12) | ((uint32_t)rx_data_buffer[2] << 4) | (((uint32_t)rx_data_buffer[3] >> 4) & 0x0F);
        uint32_t raw_temp = (((uint32_t)rx_data_buffer[3] & 0x0F) << 16) | ((uint32_t)rx_data_buffer[4] << 8) | ((uint32_t)rx_data_buffer[5]);

        data->humidity = ((float)raw_hmdt / AHT20_CONVERSION_FACTOR) * 100.0f;
        data->temperature = ((float)raw_temp / AHT20_CONVERSION_FACTOR) * 200.0f - 50.0f;
    ...
    AHT_LOGE("All #%ld read attempts failed. Returning AHT_FAIL", AHT20_READ_ATTEMPTS);
    return AHT_FAIL;
}
...
aht20_err_t aht20_soft_reset(void) {
    ...
    if (!dev_handle) {
        AHT_LOGE("Device handle not valid");
        return AHT_INVALID_ARGS;
    }

    // Checking to see if the AHT20 has been initialized or not
    if (!is_initialized) {
        AHT_LOGW("AHT already in uninitialized state");
        return AHT_INVALID_STATE;
    }

    const uint8_t soft_reset = AHT20_CMD_SOFT_RESET;

    if (i2c_master_transmit(dev_handle, &soft_reset, sizeof(soft_reset), pdMS_TO_TICKS(AHT20_I2C_TIMEOUT_MS)) != ESP_OK) {
        AHT_LOGE("Failed to transmit soft reset command");
        return AHT_FAIL;
    }
    is_initialized = false;

    vTaskDelay(pdMS_TO_TICKS(AHT20_I2C_TIMEOUT_MS));  // Delay to allow the AHT20 fully reset
