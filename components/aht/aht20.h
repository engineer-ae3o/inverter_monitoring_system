 ...
 * 
 * @note This function does not have any protection against race conditions. If you were to call it from more than 1 task, lock access to it using a mutex
 */
...
/**
 * @brief Retrieves the last cached temperature value. Does not perform any reading, hence, aht20_read must be called before it can be used, otherwise it returns stale data.
 *
...
/**
 * @brief Retrieves the last cached humidity value. Does not perform any reading, hence, aht20_read must be called before it can be used, otherwise it returns stale data.
 * 
