...
typedef struct {
    ...
    StaticSemaphore_t spi_done_sem_buffer;

    QueueHandle_t flush_queue;                 // Queue of pending flush requests
    StaticQueue_t flush_queue_buffer;
    uint8_t flush_queue_stack[ST7735_DEFAULT_QUEUE_SIZE * sizeof(st7735_flush_req_t)];

    TaskHandle_t task_handle;                  // Background processing task
    StaticTask_t task_buffer;
    StackType_t task_stack[ST7735_DEFAULT_TASK_STACK_SIZE];
...
static DMA_ATTR uint16_t pixels_buf[ST7735_MAX_WIDTH * 16] = {};       // DMA buffer
static SemaphoreHandle_t dma_semphr = NULL;                            // Semaphore to indicare DMA buffer availability
static StaticSemaphore_t dma_semphr_buffer;
...
esp_err_t st7735_init(const st7735_config_t* config) {
    ...
    driver.task_mutex = xSemaphoreCreateMutexStatic(&driver.task_mutex_buffer); // We create the mutex here because we need it to ensure thread safety
    if (!driver.task_mutex) return ESP_FAIL;

    if (xSemaphoreTake(driver.task_mutex, pdMS_TO_TICKS(ST7735_TIMEOUT_MS)) != pdTRUE) {
        ST_LOGE("Failed to take mutex during initialization");
    ...
    memcpy(&driver.config, config, sizeof(st7735_config_t));
    ...
    spi_bus_config_t bus_cfg = {
        ...
        .quadwp_io_num = -1,
        .quadhd_io_num = -1,
        .max_transfer_sz = driver.config.width * driver.config.height * 2
    };
    ...
    driver.spi_done_sem = xSemaphoreCreateBinaryStatic(&driver.spi_done_sem_buffer);
    driver.flush_queue = xQueueCreateStatic(driver.config.queue_size, sizeof(st7735_flush_req_t), 
                                            driver.flush_queue_stack, &driver.flush_queue_buffer);
    ...
    dma_semphr = xSemaphoreCreateBinaryStatic(&dma_semphr_buffer);
    if (!dma_semphr) {
    ...
    driver.task_handle = xTaskCreateStaticPinnedToCore(
                                                       st7735_task,
                                                       "ST7735Task",
                                                       ...
                                                       driver.task_stack,
                                                       &driver.task_buffer,
                                                       driver.config.task_core
    );
    if (!driver.task_handle) {
        ST_LOGE("Failed to create task");
        xSemaphoreGive(driver.task_mutex);
        st7735_cleanup_resources(); // This already handles the cleanup of the gpio and spi resources
        return ESP_FAIL;
    }
    ...
    if (ret != ESP_OK) {
        ...
        st7735_cleanup_resources();  // This cleans up all the created resources and cleans up the spi and gpio resources that have been created
        return ret;
    }
...
static void st7735_task(void* arg) {
    ...
    while (!driver.shutdown_requested) {
        ...
        if (xQueueReceive(driver.flush_queue, &req, pdMS_TO_TICKS(ST7735_TIMEOUT_MS)) == pdTRUE) {
            ...
            for (int i = 1; i <= driver.config.max_retries; i++) {

                ret = st7735_set_window(req.x1, req.y1, req.x2, req.y2);
                if (ret != ESP_OK) {
                    ST_LOGW("Attempt #%d: Failed to set pixel window", i);
                    continue;
                }
                ...
                ST_LOGW("Attempt #%d: Failed to send pixel data", i);
            }
            if (ret != ESP_OK) {
            ...
            vTaskDelay(pdMS_TO_TICKS(ST7735_TIMEOUT_MS / 5));
        }
    }
